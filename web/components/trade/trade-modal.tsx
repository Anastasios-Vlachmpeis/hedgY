...
/**
 * Controlled trade modal. Render once with `params` (null = closed). Placing an
 * order hits /api/orders → backend /orders (stocks = real Alpaca paper,
 * predictions = in-app fill at live odds), then refreshes so the account +
 * positions update. Keyed so each open starts with fresh state.
 */
export function TradeModal({
  ...
  const sig = `${params.kind}:${params.symbol ?? params.marketId}:${params.side ?? ""}`;
  return <TradeModalBody key={sig} params={params} onClose={onClose} />;
}

function TradeModalBody({ params, onClose }: { params: TradeParams; onClose: () => void }) {
  const { kind, label, price, symbol, marketId, side } = params;
  ...
  const title = kind === "stock" ? `Buy ${symbol ?? label}` : `Buy ${side} · ${label}`;
  ...
  async function submit() {
    ...
    try {
      const res = await fetch("/api/orders", {
        ...
        body: JSON.stringify({ kind, action: "buy", notionalUsd: amount, symbol, market_id: marketId, side }),
      });
  ...
  const overlay = (
    ...
    >
      ...
      >
        ...
        <label className="mt-4 block text-[12px] font-medium text-[#666666]">Amount (USD)</label>
          ...
          {QUICK.map((q) => (
            <button
              ...
              className={cn(
                ...
                amount === q ? "bg-[#181925] text-white" : "bg-[#f5f5f5] text-[#666666] hover:bg-[#ececec]",
              )}
            >
              ${q}
            </button>
...
/**
 * Buy button (ActionButton trigger) that opens the shared TradeModal. Used by the
 * market cards (Yes/No) and the stock rows (Buy).
 */
export function TradeButton({
  ...
  className,
  children,
}: TradeParams & { tone: Tone; className?: string; children: React.ReactNode }) {
  ...
  return (
    <>
      ...
      <TradeModal
        params={open ? { kind, label, price, symbol, marketId, side } : null}
